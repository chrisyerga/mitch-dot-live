...
export function NewsSection() {
  ...
  const visibleNews =
    news && !showAll ? news.slice(0, INITIAL_VISIBLE_COUNT) : news;
  const hasMore = news !== undefined && news.length > INITIAL_VISIBLE_COUNT;
          ...
          {hasMore && !showAll && (
            <div className="mt-6 flex justify-center">
              <button
                ...
                className="rounded-full border border-[color:var(--line)] bg-[color:var(--surface)] px-5 py-2.5 text-sm font-semibold text-[color:var(--fg)] hover:border-[color:var(--accent2)]"
              >
                Show More
              </button>
            </div>
